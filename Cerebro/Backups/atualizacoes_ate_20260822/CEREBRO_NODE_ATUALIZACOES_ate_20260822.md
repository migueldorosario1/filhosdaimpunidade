# ARQUIVO rotacionado de /home/migueldorosario/Downloads/Antigravity Google/Cerebro/CEREBRO_NODE_ATUALIZACOES.md
...
## 2026-08-21 ~22:40 BRT — ZCode/DeepSeek — Post 266972 (Lula/Valadares): foto real de hoje + no-home manual + auditoria de autoria; BUG "ministro fantasma" aberto

- **Ordem do Miguel (22:14):** foto de Lula HOJE em Minas para o post + categoria no-home + "quem fez isso?" (post muito fraco).
- **Foto:** capa oficial do álbum "21.08.2026 - Visita às Obras de Duplicação da Ponte São Raimundo (BR-116/251/MG)" do Flickr **Lula Oficial** (Ricardo Stuckert, CC BY-SA 2.0), validada por visão (9/10), attachment 266980 como destacada com alt+caption creditados; meta `_cafezinho_img_check` atualizada com trilha da LAURA-AGY preservada.
- **no-home:** `wp post term add 266972 category no-home` (slug, term 20699) — post FORA da home (provado). Nota: não conflita com a regra viva de 13/08 (V4 não AUTO-aplica no-home) — foi curadoria manual pontual por ordem direta do Miguel.
- **Quem fez:** redigido pelo **V4 Nacional** (`zizi_job_id v4d_nacional_6855a4d21e2c4651`); imagem aprovada e publicado pela **LAURA-AGY** às 21:28 (meta `_cafezinho_img_isenta`, user gabrielbarbosa/5735). Texto é pré-evento (futuro) publicado 6h depois.
- **BUG aberto:** `CEREBRO_NODE_BUGS_ATIVOS.md` BUG-20260821-V4NACIONAL-MINISTRO-FANTASMA — post cita "ministro George Santoro"; fontes oficiais do dia dizem **Alexandre Silveira**. Correção aguarda decisão do Miguel.
- Tema Duplo: `Foruns/forum_20260821_post_lula_valadares_foto_no_home.md` + `Memorias/memoria_20260821_post_lula_valadares_foto_no_home.md`.

## 2026-08-21 ~17:25 BRT — ZCode/GLM-5.3 — Recarga Miguel (OpenAI+DeepSeek) DESTRAVOU a redação V4; cooldown inocente; resíduo = proxy IPRoyal 402

- **Health check pós-recarga:** deepseek OK (US$ 18,51 medidos no `/user/balance`), openai gpt-4o-mini + gpt-5.5 OK, glm/qwen/assemblyai OK; kimi/moonshot segue suspenso. NO_PROXY do NYC já cobre api.deepseek.com/api.openai.com → recarga chegou direto.
- **Cooldown (pergunta do Miguel): nada a desligar.** O `hourly_quota` de 55min do worker é cadência global por SUCESSO (design, `--force` ignora); `falhas_modelos_runtime.json` só tem quirk `sem_temperature` do claude-opus-4-8.
- **Prova de destravamento:** rodada manual `v4_vertical_draft_worker.py geopolitica` → **draft 266937 criado** (LLM redigiu; imagem pending por cota IA 50%, não LLM).
- **⚠️ Resíduo aberto:** IPRoyal (proxy do chaves.sh) devolve `402 Payment Required` no túnel → quebra coringa AssemblyAI + Flickr (fotos ao vivo). Pendente Miguel: recarregar IPRoyal × expandir NO_PROXY. Nacional em stall por `no_candidate` (coleta pol fraca) — não é LLM.
- **Registro:** `Foruns/forum_coringa_assemblyai_cascata_llm_20260816.md` Adendo 2.

## 2026-08-21 ~14:30 BRT — ZCode/GLM-5.3 — Comentários automáticos DESLIGADOS (comentarista V4 + enxame + engajamento da manchete)

- **Ordem do Miguel:** "desliga o agente comentarista no cron e também desliga os comentários no agente manchete".
- **Feito no NYC (com backups `*.bak_pre_comentarios_off_20260821`):** (1) cron `7,37` do `agente_comentarista_v4.py` comentado; (2) cron `*/10` do `disparador_enxame.py` comentado — sem isso o enxame continuaria (era o gatilho da manchete + posts nacionais cat 22); (3) `agente_manchete.py` deixou de disparar o `robo_super_engajamento.py` (guard `MANCHETE_COMENTARIOS=1`, default off; py_compile OK). Cron do agente manchete segue ATIVO (0 */2).
- **Revogações implícitas:** "regra permanente" de 17/08 (enxame automático em manchete de política nacional) e regra 12/08 (todo post nacional comentado) — ambas suspensas enquanto os crons estiverem comentados.
- **Nenhum processo de enxame ativo** no momento do corte. Reativação: descomentar as 2 linhas do crontab NYC + `MANCHETE_COMENTARIOS=1`.
- **Registro:** `CEREBRO_NODE_COMENTARISTA.md` (banner ⛔ no topo) + `Foruns/forum_enxame_266483_ciro_mossad_cap_500_20260818.md` Adendo 1.

## 2026-08-21 ~14:15 BRT — ZCode/GLM-5.3 — Health check cadeia LLM V4: 4/7 providers mortos por crédito

- Smoke real por provider (`nucleo_llm.py`, cofre local): 🟢 glm-4.5-flash · qwen-plus · assemblyai (coringa). 🔴 deepseek (402) · kimi/moonshot (429 conta suspensa) · openai gpt-4o-mini + gpt-5.5 (429 sem créditos). V4 não para (cascata cai no glm), mas tier superluxo entrega glm na prática. Detalhe: `CEREBRO_NODE_CHAVES_E_LLMS.md` (seção 21/08) + `Foruns/forum_coringa_assemblyai_cascata_llm_20260816.md` Adendo 1. Pendente Miguel: recargas ou aceite.
...
## 2026-08-20 21:15 BRT — Manus — Primeiro snapshot externo de audiência

- **Fonte:** Revista Fórum, relatório de últimos 30 minutos encerrados às 20:00 BRT, enviado por Miguel no chat e preservado como dado declaratório da fonte.
- **Registro:** 3.240 usuários ativos, pico simultâneo de 524, 19.688 visualizações e ranking de dez matérias. Persistência normalizada no `cafezinho-dashboard` sob `revista-forum`, sem mistura com tabelas GA4 do Cafezinho.
- **Comparação futura:** repetir somente com fonte, janela e horário explícitos. Não tratar números absolutos entre portais como diretamente comparáveis sem contexto metodológico.
- **Tema:** `Foruns/forum_snapshot_externo_revista_forum_20260820.md`.
...
- **21/08/2026 17:39 — ZCode Miguel (GLM-5.3, ronda ponte):** canal `cafezinho-wp-write` da Laura estendido com `publish` + `schedule` (whitelist 7→9 ops; wrapper + write-query.php; gmt derivado server-side; backups `*.bak_pre_publish_20260821_1426`), por ordem direta do Miguel (13:00) que revoga o "corrigir sim, publicar não" e institui o pacto de dupla checagem CL×AGY (AL-021/022, CL-018). Testado ponta a ponta com posts descartáveis; gates do site intactos. ZM-20260821-001 na ponte + ciência ao Claude Miguel no canal Trindade (exclusividade de publicação do CM, Regra 10 provisória, superada). Tema Duplo: adendo no fórum da ponte + `Memorias/memoria_pd1_canal_write_laura_publish_schedule_20260821.md` + ponteiro no NODE_ARQUITETURA §0.
- **21/08/2026 ~23:45 BRT — ZCode (GLM-5.3):** novo fórum `Foruns/forum_mapa_autoria_posts_v4_v5_humanos_20260821.md` — mapa de autoria dos posts do canônico (V4=meta zizi v4d_* autor 5786; V5=Motor AGY pool 5786/5742/5785/5470 sem meta desde 20/08; YouTube=meta nomes_check; repetidor=5470; Gabriel=5780/5735; Miguel=2018 James2017) com contagens 15-21/08 e queries SQL. Tema: curadoria/autoria (irmão dos fóruns de manchete 12/08 e transição V5 20/08).
- **22/08/2026 01:30 BRT — ZCode (GLM-5.3):** criado o **Cofre SSH cifrado em 3 destinos** (ordem do Miguel): `Cerebro/Cofres/cofre_ssh/` (cofre_ssh.sh + LEIA-ME) — GPG simétrico AES-256/KDF máx; blob `cofre_ssh_v1.tar.gpg` espelhado em pendrive 2079-8A26 (`cofre_segredos/`) + GitHub (via trilho) + GDrive (`drive:Cofres/cofre_ssh/`); passphrase NUNCA em arquivo (protocolo palavra-ao-LLM com `abrir` em /dev/shm + `fechar` + `esquecer` limpando transcripts). Selftest 3/3 + canais provados c/ blob demo (removido depois). Blob real aguarda o Miguel rodar `selar` com a palavra dele. Tema Duplo: `Foruns/forum_cofre_ssh_criptografado_3destinos_20260822.md` + `Memorias/memoria_cofre_ssh_criptografado_3destinos_20260822.md` + seção nova no NODE_COFRE_CHAVES.
- **22/08/2026 ~11:05 BRT — ZCode (GLM-5.3):** sprint Mapa de Sinais de autoria (ordem Miguel): (1) documento vivo `Foruns/MAPA_SINAIS_AUTORIA_CAFEZINHO.md`; (2) mu-plugin `cafezinho-origem-post.php` no canônico — meta `_cafezinho_origem` (via+UA) em todo post novo + API `GET /wp-json/cafezinho/v1/autoria` (app password "Integracoes-Autoria" espelhada em 3 cofres); (3) página `/v6/autoria` no Painel CCTV; (4) seção "🧭 Quem fez o quê" no Baleia Azul (08:00/19:30). Tema Duplo `forum_/memoria_mapa_sinais_integracoes_baleia_cctv_20260822`. Descoberta: fronts 190.89.239.244/.31 concentram todo tráfego — UA (não IP) identifica ferramenta; Manus publica dentro do UA Antigravity/1.0 com author=2018 (caso 267017).
- **22/08/2026 ~10:25 BRT — ZCode (GLM-5.3):** credencial `AUTORIA_*` (app password Integracoes-Autoria, API de autoria) registrada no NODE_COFRE_CHAVES e espelhada nos cofres unificados que faltavam (Tencent root+ubuntu, NYC; backups .bak_pre_autoria_20260822; verificação por nome de chave 3/3 em todos). Pendência: Alibaba segue inacessível (host key, desde 16/08).
- **22/08/2026 ~10:35 BRT — ZCode (GLM-5.3):** /v6/autoria enriquecido com audiência (ordem Miguel): views GA4 por post (matching por slug; campo slug adicionado à API), cards autoria×views+média+melhor post, ranking publicação × performance, coluna Views na tabela (fechado até ontem). Adendo 1 no fórum do mapa de sinais.
- **22/08/2026 ~10:35 BRT — ZCode (GLM-5.3):** ZM-20260822-002 na ponte (relato sprint + PEDIDO DE CHECK + proposta de emenda ao contrato: rastro de autoria obrigatório / API fonte única / mudanças passam pela ponte). Integração imediata do PROTOCOLO_SSH_AGENTES_v1 ao classificador (11 identidades 5788-5798 → "Agente <nome> (SSH v1)", provado c/ draft zcode_miguel). Fix bug GMT na API (drafts wp-cli invisíveis → pending visíveis). Regra viva §128 (coordenação pela ponte) registrada.

- **22/08/2026 10:30 BRT — ZCode/GLM-5.3:** Diagnóstico V4 Nacional (ordem Miguel "análise de saúde do V4"): coleta morta desde 20/08 pelo proxy IPRoyal 402 (RSS+GoogleNews via env) + gap freshness Brave na politica → nacional 0 posts/dia. Fix aplicado no NYC com backups (coletor.py + flickr_live.py trust_env=False; politica no freshness pw); provado ao vivo (16 candidatas, draft 267050, 145 fotos Flickr). Tema Duplo: `Foruns/forum_v4_nacional_diagnostico_coleta_proxy_fix_20260822.md` + `Memorias/memoria_v4_nacional_diagnostico_coleta_proxy_fix_20260822.md`. Aguardam Miguel: IPRoyal, sweep pending, Pacote Qualidade (tese/vilão).
- **22/08/2026 ~10:12 BRT — ZCode (GLM-5.3):** adendo do cofre SSH por ordem do Miguel ("tudo tem que estar no Cérebro, indexado"): explicação em LINGUAGEM SIMPLES gravada no fórum (ADENDO 1) + no LEIA-ME do cofre (seção "Explicação simples") + entrada nova no `Foruns/INDICE_FORUNS_SEMANAL.md` (§ Cofre SSH cifrado em 3 destinos). Cofre segue aguardando o Miguel rodar `selar`.
- **22/08/2026 ~10:45 BRT — ZCode (GLM-5.3):** indexação completa do sprint Mapa de Sinais (Camada 2): NODE_PUBLICACAO_WP_CAFEZINHO (§mapa-sinais-autoria novo), NODE_OBSERVABILIDADE (§22 /v6/autoria), NODE_BALEIA_AZUL (seção automática do boletim) + INDICE_FORUNS_SEMANAL (seção nova no topo). Já estavam: NODE_COFRE_CHAVES (credencial AUTORIA_*), GOVERNANCA §128 (coordenação pela ponte).

- **22/08/2026 ~10:50 BRT — ZCode/GLM-5.3:** Portão anti-repetição criativo no worker V4 (ordem Miguel "não repetir matéria; últimos 50 posts; mesmo assunto só com tese/ângulo/título diferentes"): `recent_published_titles_n(50)` + `anti_repetition_gate` com juiz LLM (regra do publisher no prompt; fallback conservador) + contexto de posts recentes no redator. Backup `.bak_pre_antirepeticao50_20260822` no NYC. Provas: repetida bloqueada c/ motivo; fresca liberada; E2E nacional = post 267050 publicado 10:28 BRT (1º desde 21/08). Credencial AUTORIA_* da sessão paralela conferida no Cofre (1ª senha revogada/recriada — ok). Tema Duplo: adendo no fórum/memória do diagnóstico V4.
- **22/08/2026 ~10:50 BRT — ZCode (GLM-5.3):** SIGILO (ordem Miguel): (1) fechado vazamento preexistente — REST pública expunha zizi_job_id/nomes_check/_agente_origem etc. no canônico+espelho → filtro rest_prepare_post prio 9999 esconde metas internas sem auth (agentes autenticados intactos); (2) /v6/autoria do CCTV (painel é público no IP Tencent) protegido com Basic Auth (401/200 provados; creds AUTORIA_PAINEL_* nos cofres; resto do painel intacto — proteger o resto = decisão pendente do Miguel); (3) meta _cafezinho_origem nunca exposta (provado). Adendo 3 no fórum do mapa de sinais.

- **22/08/2026 ~11:00 BRT — ZCode/GLM-5.3:** ordem Miguel "fotos jornalísticas, não oficiais": post 267050 trocado (retrato → ato de BH, visão 8/10) + Plano C permanente no flickr_live (foto jornalística recente quando casamento A/B falha; prova com a pauta real). Backup .bak_pre_planoC_20260822. Fórum do diagnóstico V4 ADENDO 2.
- **22/08 11:39 — ZCode/Qwen 3.8:** simulação LOCAL do card quadrado 1:1 do Twitter (gerador_card_top1_quadrado.py + cards/04_twitter_quadrado_fab_golpe_SIMULACAO.jpg); NADA publicado; fio de 2 desenhado aguardando aprovação (fórum plano social, adendo 13).
- **22/08/2026 12:00 BRT — ZCode/Qwen 3.8:** integração total FdI↔Manus (ordem Miguel): missão FdI permanente na ponte_manus_miguel (MISSAO_FDI_MANUS.md + PROMPT_MANUS_FDI_v1.md + ZM-20260822-005) · GitHub canônico do FdI confirmado/auditado (sem segredos rastreados) + repo local alinhado ao canonical (pull 06945f4) · backup horário do repo → gdrive:filhosdaimpunidade/ (script+cron novos) · sync do Cérebro patcheado (ponte_manus_miguel fora do copy_tree, backup .bak_pre_ponte_manus_exclusao_20260822). Tema Duplo forum_/memoria_fdi_integracao_manus_github_gdrive_20260822. Aguarda Miguel colar o prompt no Manus.
- **22/08 11:57 — ZCode/Qwen 3.8:** assinatura Kimi Code oferece `k3-256k` (K3 c/ contexto 256K, gasta menos quota da janela) — verificado ao vivo via /models; adicionado ao seletor do ZCode (backup config.json.bak_pre_k3_256k_20260822); linha nova no CATALOGO_MODELOS_LLM (seção Moonshot).
- **22/08/2026 ~11:50 BRT — ZCode (GLM-5.3):** /v6/autoria trocou Basic Auth por link secreto `?k=AUTORIA_TOKEN` + cookie 1 ano (404 sem token; ordem Miguel "aberto sem login"); token `AUTORIA_TOKEN` nos cofres; nginx sem auth na rota. Adendo 4 no fórum.

- **22/08/2026 ~13:15 BRT — ZCode/GLM-5.3:** boost de produção de fim de semana (ordem Miguel): 6 crons sáb/dom no NYC (nacional horária extra, economia/cultura */2h, meio/esporte/saúde */4h) — expiram sozinhos na segunda. Duplicata de instalação corrigida do backup; estado final 6 linhas. Prova: draft 267079. Proteções anti-repetição/fotos jornalísticas seguem ativas. Fórum diagnóstico V4 ADENDO 3.

- **22/08/2026 ~13:40 BRT — ZCode/GLM-5.3:** mapa das cascatas LLM por fase do V4 documentado (pergunta Miguel; correção: redação principal = rota luxo deepseek→**gpt-5.5**→claude-opus-5, PROVADO nos logs de hoje; glm-5-turbo é só o 1º dos gates JSON). Fórum diagnóstico V4 ADENDO 4.

- **22/08/2026 ~16:00 BRT — ZCode/GLM-5.3:** pesquisa (ordem Miguel, sem alterações): arquitetura V4 "LLM em tudo" LOCALIZADA — `/root/v4_labs/contratos/mapa_v4_contexto_llm.json` + `v4_rotas_llm_limpas_v1.json`: 7 funções LLM (curadoria/redação gpt-5.5/revisão/auditoria/fact_check c/ websearch/imagem/repetidor), pipeline separado em stages, status **rascunho_dry_run** (nunca substituiu o worker determinístico). Fórum diagnóstico V4 ADENDO 5.

- **22/08/2026 ~16:45 BRT — ZCode/GLM-5.3:** fila de fallback LLM caro→barato + cooldown de volta rápida IMPLEMENTADA no v4_labs (ordem Miguel): llm_cooldown.py novo (90s×2ⁿ teto 15min, sucesso limpa), ganchos no model_router/llm_adapter, rotas limpas com 8 degraus (fact_check topo Perplexity sonar-pro c/ websearch), piso de qualidade 5→4 p/ reservas. Provas ao vivo: redação GPT-5.5, queda ordenada até moonshot, nunca cair. Fórum: forum_v4_labs_subida_pipeline_llm_tudo_20260822.

- **22/08/2026 ~17:25 BRT — ZCode/GLM-5.3:** ordem Miguel: regime de produção monitorada com AUTOAPRENDIZADO na ronda V4 30/30 — 24h ou até 3 rondas 🟢 seguidas; cada incidente = causa raiz + fix com backup + prova + LIÇÃO no fórum da missão. Automação 0b3f90de atualizada (contagem de estabilidade no relatório; Telegram). Fórum subida §DIRETRIZ.

- **22/08/2026 ~19:10 BRT — ZCode/Qwen 3.8:** MOKA READER — telemetria de gastos de IA (ordem Miguel): ledger local IndexedDB `moka_telemetry` (tokens+custo de TODA tarefa livro/vídeo/TTS) + pop-up UsageToast (USD e moeda local, "não quero mais ver isso") + prefs (sempre/acima de X/nunca + trava de tokens por tarefa que NUNCA deixa o app travar) + página `/telemetria` (por IA/tarefa/modelo, marcador moeda local, calculadora, CSV) + ícone 🧩 troca modelo por chave + 💰 saldo (DeepSeek via API; demais = painel oficial) + i18n 12/8 idiomas. Commit+push `fc63cb7` (build verde pós-rebase). Tema Duplo `forum_/memoria_moka_telemetria_gastos_ia_20260822` + INDEX_MOKA_MASTER.

- **22/08/2026 ~20:15 BRT — ZCode/Qwen 3.8:** MOKA READER — rodada 2 do feedback do Miguel na telemetria (commit+push `a3db3c9`, produção verificada): 💰 saldo removido por completo (nenhum provedor expõe crédito); editor de modelo 🧩 consertado — causa raiz era render dentro da coluna flex estreita, agora abre em largura total abaixo do card com lista automática e seleção em 1 clique; ícone 📊 na topbar de Capa/estante/video/ajuda + banner destacado nas configurações; `/telemetria` reescrita como página de controle "Suas IAs" (topbar padrão c/ idioma/fechar, chaves registradas c/ troca de modelo e gasto por IA, gastos por uso, tabela de preços); corrigido 📊 duplicado no título. Adendo fórum+memória `*_moka_telemetria_gastos_ia_20260822` + monitor ✅.

- **22/08/2026 ~21:40 BRT — ZCode/Qwen 3.8:** MOKA READER — ESPELHO DE EXPERIMENTOS NO AR (ordem Miguel): projeto Vercel `moka-espelho` + repo GitHub `migueldorosario1/moka-espelho` + branch local `espelho` (remote `mirror`); URL https://moka-espelho.vercel.app; 10 env vars espelhadas (SITE_URL apontando pro espelho); guard de domínio no GA4 (analytics só no canônico — provado: espelho 0, canônico 2); auto-deploy de produção provado ponta a ponta; canônico intacto. Descoberta: API pública da Vercel não seta production branch (repo próprio resolve). Tema Duplo `forum_/memoria_moka_espelho_experimentos_20260822`.
- 22/08 ~21:10 (ZM/Qwen): AGENTE YOUTUBE anti-desperdício — fórum+memória novos (`forum_/memoria_agente_youtube_antidesperdicio_20260822`), manual loops §10, NODE_AGENTES adendo, ZM-20260822-175 nos 3 canais. Cache de transcrição + pendentes + fix cascata gerar_json + breaker "quem não publica não transcreve" (E2E provado). GSN V2 NYC parado por decisão (cron sumiu 19/08; 27 drafts).

- **22/08/2026 ~21:05 BRT — ZCode/Qwen 3.8:** MOKA READER — INCIDENTE + ROLLBACK (ordem Miguel): tradução de livro quebrou no canônico com a telemetria de hoje; causa raiz = `stream_options.include_usage` no streaming OpenAI-compatible (commit fc63cb7). Rollback TOTAL do dia na main (reverts 335e18c+96f0713 → árvore = fc84138 de 19/08; produção verificada: capa 200, /telemetria 404). Todo o trabalho de hoje agora vive SÓ no espelho (moka-espelho.vercel.app), incluindo o fix da tradução (fcf29e7: streaming sem stream_options). Regra nova do fluxo: reforma nunca entra direto no canônico — espelho primeiro, merge depois de aprovado. BUG-20260822-MOKA-TRADUCAO-STREAM-OPTIONS em BUGS_ATIVOS; fóruns telemetria §Adendo 3 + espelho (adendo).

- **22/08/2026 ~21:30 BRT — ZCode/Qwen 3.8:** MOKA ESPELHO — CRASH CLIENT-SIDE CORRIGIDO (reporte Miguel "não entrou a página espelho"): as 10 env vars do projeto espelho eram blobs cifrados (espelhamento via API da Vercel sem plaintext — vars `sensitive` não são legíveis de volta). `NEXT_PUBLIC_SUPABASE_URL` inválida → AuthGate lançava exceção → toda página com login quebrava. Reespelhadas 9 vars com valores reais (Supabase = público, extraído do bundle canônico; servidoras = cofres locais); GITHUB_TOKEN_MOKA sem fonte → removida; rebuild 0615e5f verificado (capa/sobre/telemetria abrindo). Gotcha permanente registrado em BUGS_ATIVOS + fórum/memória do espelho.

- **23/08/2026 ~01:05 BRT — ZCode/GLM-5.3:** PLANO TOTAL DE SEGURANÇA & CONTINGÊNCIA aberto (ordem Miguel ~00:55 — cobre Cafezinho, Cérebro, GitHub, Vercel, GoDaddy, GDrive, servidores, chaves, Telegram/WhatsApp/Gmail, redes sociais). Tema Duplo `forum_/memoria_plano_seguranca_contingencia_20260823` + **NODO NOVO** `CEREBRO_NODE_SEGURANCA_CONTINGENCIA.md` (Camada 2). Agenda de trabalho **48h em 48h** (S1–S10, 25/08→12/09) com automação ZCode disparando 10:00 (prepara a rodada, grava adendo no fórum e avisa o Miguel no Telegram). S1 = Identidade raiz (2FA/recuperação). Zero mudança de produção.

- **23/08/2026 ~10:50** (ZCode/GLM-5.3): pauta editorial "4 artigos Bangu/Lula 22/08" aberta — plano + artigo 1 (Eduardo Paes, campanha casada com Lula) + notas com fontes TSE/Datafolha; Tema Duplo `forum_4_artigos_bangu_lula_20260823.md` + memória par; catalogado no NODE_PUBLICACAO_WP_CAFEZINHO. Artigos 2-4 após revisão do Miguel.
- **2026-08-23 16:15 — ZCode (GLM-5.3):** pesquisa DeepSeek Harness (DSH) a pedido do Miguel → fórum `Foruns/forum_deepseek_harness_dsh_pesquisa_20260823.md` + memória + seção nova no `CEREBRO_NODE_CHAVES_E_LLMS.md`. Parecer: DSH é harness da mesma categoria do ZCode (preview, MIT, multi-provedor); Dell já atende pré-requisitos; vale laboratório, não substituição. Conta DeepSeek API seguia 402 (21/08) — recarregar p/ testar.
- **24/08/2026 08:31 BRT — ZCode/GLM-5.3:** AUDITORIA DO GASTO DEEPSEEK (ordem Miguel + zip oficial 18-24/08): total **US$ 182,97** — 55% chave "moka reader" (BYOK do Miguel no navegador, v4-pro, ~418k tokens/req: Reader reenvia o livro inteiro por página traduzida), 41% "z code api" (pico 18-19/08, já contido pelo failover→GLM). Gateway da casa (Tencent) quase zerado (6 chamadas/20 dias; último débito de pontos 04/08). **Auditoria de chaves expostas no Moka: LIMPA** (repos públicos varridos no git histórico todo + AAB da Play Store descompactado). Rio Carta parado desde 18/08 confirmado — matérias sendo geradas (1,63M tokens output na chave dos temáticos da Laura) e presas sem publicar (padrão Discover Brazil). Rotação de chaves decidida pelo Miguel — mapa completo de onde atualizar cada uma no fórum. Tema Duplo: `Foruns/forum_auditoria_gasto_deepseek_chaves_moka_20260824.md` + `Memorias/memoria_auditoria_gasto_deepseek_20260824.md`.
- **24/08/2026 ~09:30 BRT — ZCode/GLM-5.3:** AUDITORIA OPENAI + SUPERPRODUÇÃO V4 (ordem Miguel, CSVs oficiais 20-24/08): US$ 39,03 na org "O Cafezinho" — pico 23/08 US$ 21,26 causado pela decisão da ronda 22/08 de a "V4.1 sombra carregar o volume" (gpt-5.5+web_search gerando em paralelo sem gate de publicação). Motor: crons V4 no NYC (`v4_labs`, geo a cada 30min). Superprodução medida no WP: >500 rascunhos, 361 pending, 51 rascunhos novos num dia vs ~28 publicados/dia. REGRA NOVA do Miguel: **ZERO produção no Dell — tudo em servidor** (pendência: migrar crons youtube_cafezinho). Tema Duplo: `Foruns/forum_auditoria_gasto_openai_v4_superproducao_20260824.md` + `Memorias/memoria_auditoria_openai_v4_20260824.md`. No mesmo dia: furo DeepSeek/moka fechado (aba zombie no Chrome morta pela rotação às 08:52; saldo estancado US$ 4,09).
- **24/08/2026 ~09:30 BRT — ZCode/GLM-5.3:** EXECUÇÃO da racionalização (ordem Miguel ~09:05): **V4 desligado no NYC** (17 workers; backup crontab `/root/backup_crontab_pre_v4off_20260824_0912`), **V4.1 vira o pipeline oficial** (4 verticais a cada 2h; coletor geo 30min→1h; coletor+intake mantidos como pauteiro barato). **Moka: conserto estrutural do gasto passivo** — commit `11478fc` no espelho: tradução de livro pausa sozinha em aba oculta + fechar janela = pausar (raiz dos US$ 101 invisíveis). Plano de Racionalização R1-R5 no adendo 1 do fórum da auditoria OpenAI (R1b lixeira de 500 rascunhos e R4 migração YouTube aguardam OK/janela do Miguel).
- **24/08/2026 ~09:35 BRT — ZCode/GLM-5.3:** FIX MOKA NOS DOIS AMBIENTES (ordem Miguel "corrigir urgente o canônico e o espelho"): pausa-em-segundo-plano publicada no espelho (`11478fc`) e no canônico via cherry-pick cirúrgico (`1c6b31b` na main, sem levar os experimentos do espelho). Ambos com deploy Vercel automático.
- **24/08/2026 ~09:55 BRT — ZCode/GLM-5.3:** MOKA COM TELEMETRIA ROBUSTA NOS 2 AMBIENTES (ordem Miguel "toda ação registrada e anotada na página de telemetria"): canônico `41de000` (merge completo do espelho: /telemetria com hora/tarefa/modelo/tokens/custo/status/notas + trava + CSV; nota nova `vol X/Y · pág Z` no job do livro) e espelho `92b24f4`. Fóruns: forum_auditoria_gasto_deepseek_chaves_moka_20260824.md (adendo 2).
- **24/08/2026 ~10:00 BRT — ZCode/GLM-5.3:** REGRA DE PEDRA §118 gravada (ordem Miguel): todo uso de LLM exige telemetria robusta+redundante (local durável + push pro hub CCTV), visível online na página de custos; sem telemetria não sobe em produção. Varredura do sistema e arquitetura do hub em andamento (fórum a seguir).
- **24/08/2026 ~10:10 BRT — ZCode/GLM-5.3:** VARREDURA DE TELEMETRIA LLM concluída (agente Explore): espinha dorsal JÁ EXISTE (telemetria_api.py + gerenciador_tokens/banco_custos.jsonl + governanca_financeira (redundância) + painel CCTV /v6/custos no Tencent :8084). GAPS: redator V4/V4.1 sem registro (o maior gastador!), nucleo_llm/visao, auditor_titulos, exportação ZCode/Tencent pro hub. Plano E1–E5 gravado em `Foruns/forum_telemetria_redundante_llm_hub_cctv_20260824.md`. E1 (instrumentar redator) na próxima sessão.
- **24/08/2026 ~10:25 BRT — ZCode/GLM-5.3:** U1 EXECUTADO (plano de urgência da telemetria, ordem Miguel): redator V4/V4.1 instrumentado no NYC (backups .bak_telemetria_20260824, selftest ✅ — maior gastador agora grava no banco_custos a partir do próximo ciclo). Planos URGÊNCIA U2-U5 (endpoint ao vivo + push + página despesas ao vivo + exportadores) e MÉDIO PRAZO M1-M6 (nomes de chave, redundância tripla, reconciliação, tetos) em `Foruns/forum_plano_telemetria_urgencia_painel_ao_vivo_20260824.md`. Meta: cobertura ≥95% em 7 dias.
- **24/08/2026 ~11:15 BRT — ZCode/GLM-5.3:** PAINEL DE DESPESAS AO VIVO NO AR (ordem Miguel "hoje, em servidor"): /v6/custos/ao-vivo no cctv-v6 (Tencent :8084) + API /telemetria/v1/ultimos, alimentado por flusher */1min do NYC (U1-U4 da urgência cumpridos em servidor; zero Dell). Bug extra corrigido: sync */15 mandava o banco de JULHO pro painel (mês travado). Testado com dados reais. Próximos: exportadores Tencent/159.89/Laura + reconciliação semanal.
- **24/08 11:07→11:5x — ZCode (GLM-5.3):** auditoria agentes YouTube (Cafezinho Dell + 8 temáticos + GSN V2 NYC), ordem do Miguel. Números de custo×aproveitamento provados (stats.jsonl US$ 58,26 Dell; sqlite NYC US$ 148,16 GSN → 18 publicados, US$ 8,23/post; 34 descartados por vencimento; rejeitados por qualidade = 46% do gasto Dell, gate pós-pagamento). Bugs novos: yt-dlp sem JS runtime no NYC; duplicata de transcrição Dell×NYC. Tema Duplo: Foruns/forum_auditoria_agentes_youtube_transcricoes_20260824.md + Memorias/memoria_auditoria_agentes_youtube_transcricoes_20260824.md. Catalogado em CEREBRO_NODE_AGENTES.md.
- **24/08 11:25→13:35 — ZCode (GLM-5.3): EXECUÇÃO da correção YouTube (ordem Miguel "Laura fora dos temáticos; GSN pode publicar; pode corrigir tudo"):** temáticos youtube OFF (4 configs, backups) + Laura avisada (ZM-210); gsn_fila ZERADA — 10 matérias publicadas no globalsouth.news (9 traduzidas PT→EN com transcrições já pagas, commit 2e90f5a, provas HTTP 200) + banco NYC marcado; causa-raiz patcheada no materializador NYC (idioma do material segue o vídeo); consumidor automático da fila instalado (cron Dell 12:30); pré-filtro de duração anti-desperdício no transcritor do Dell (provado: 19s → custo 0); deno instalado no NYC (yt-dlp runtime ok; anti-bot pendente IPRoyal). Adendos em Foruns/forum_auditoria_agentes_youtube_transcricoes_20260824.md §8.
- 24/08 13:40 · ZCode/GLM-5.3 · Fórum+Memória `analytics_queda_visitas_online_cafezinho_20260824` em OBSERVABILIDADE — checagem GA4: sem incidente; queda percebida = pós-pico domingo + intraday incompleto + produção reduzida.
- 24/08 14:05 · ZCode/GLM-5.3 · RETIFICAÇÃO no fórum analytics_queda_visitas_online (GA4 realtime SUBNOTIFICANDO — site são, provas: nginx humanos normal + visita Chrome real invisível + MP hit invisível em 2 propriedades) + CONTADOR REDUNDANTE instalado (log nginx com CF-Connecting-IP + contador.sh + cron */5 no cafezinho-wp; gotchas: sites-enabled é cópia, conf.d após sites-enabled).
- 24/08 14:35 · ZCode/GLM-5.3 · CCTV ganhou /v6/audiencia-redundante (contador próprio × GA4 lado a lado, histórico jsonl para sempre; push do site pela porta 80 com X-Token; adendo 3 do fórum analytics). Leitura estreia: contador 305 × GA 27.
- 24/08 14:35 · ZCode/GLM-5.3 · **Agenda Google do Miguel conectada por OAuth** (`logis-agenda`, Desktop, loopback): refresh_token no cofre `Outros/chaves/Google Agenda/`; 11 eventos do Programa de Ação ILS criados via API (script idempotente em logis/scripts/). Retifica nota de 24/08 (3) do fórum do ILS (acesso antes negado). Registro: NODE_COFRE_CHAVES §Google Agenda + adendo 24/08 (4) no fórum do ILS.
- 24/08 14:45 · ZCode/GLM-5.3 · FAROL batizado + página v3 + login auth_basic (cofre cofre_farol). ADENDO 5 fórum analytics.
- 24/08 14:46 · ZCode/GLM-5.3 · ADENDO 6: pendência de investigação da causa do GA4 travado (roteiro 5 passos; amostra 14:4x: GA 27-55 vs FAROL 305).
- 24/08 14:48 · ZCode/GLM-5.3 · FAROL: auth removido (página aberta, ordem Miguel); cofre farol DEPRECADO. ADENDO 7.
- 24/08 14:52 · ZCode/GLM-5.3 · ADENDO 8: pesquisa de recorrência das falhas GA4 realtime (grandes ~2-4×/ano 2024-26; queixas semanais; threads ativas na mesma semana do nosso caso; hipótese throttle-por-spike p/ investigar c/ o recorde de domingo).
- **24/08/2026 19:40 BRT — ZCode/GLM-5.3:** PLANO DE TRABALHO da telemetria de AUTORIA dos posts aberto (ordem Miguel): `Foruns/forum_plano_telemetria_autoria_posts_20260824.md` — 7 problemas provados no dia (conta Redação compartilhada, IP invisível via NAT, meta frágil do V4.1, correção sem auditoria, cookie de sessão, sem histórico de edição, sem série por post) → fases F0 (alívio 25/08) → F1 fundação de identidade (contas individuais + carimbo no plugin + autor explícito dos publicadores) → F2 IP/geo real → F3 painel rico + auditoria + validação cruzada → revisão 31/08 com métricas (≥95% autor rico; Gabriel×Rhyan separados). Aguardam Miguel: contas WP individuais, rota do IP (CF × real_ip), select de identidade leve.
- **24/08/2026 22:30 BRT — ZCode/GLM-5.3:** BANCO DE VISITAÇÃO DO FAROL passa a morar também NO CÉREBRO (ordem Miguel — "material importante, pode guardar no Cérebro, que é backupado"): novo diretório `Cerebro/Dados/FAROL/farol_audiencia.db` (sqlite, série de 30min: online×GA4×posts). Sincronização automática `15,45 * * * *` (Dell puxa do tencent — operação de backup; o Cérebro cuida do resto: GitHub 15min + B2/Drive diário). Fonte original continua no tencent (redundância dupla). Painel público do CCTV: http://43.156.151.165 (nginx→8084).
- **25/08/2026 00:35 BRT — ZCode/GLM-5.3:** DEEPSEEK ROTACIONADA (Regra Nº 4 cumprida; chave "Cafezinho Canonico" nova, sk-f6f1c***8762, ID c8d5281d): substituída a DEEPSEEK_API_KEY (antiga sk-e36d25*** "sites temáticos", revogada) nos 3 cofres (tencent /root/.env.unificado + 2 .env.unificado Dell) com backups .bak_pre_dsk_new_20260825. Monitor: ✅ ok HTTP 200 (antes 🔑 401). Nota: valor jamais registrado em texto; sufixo apenas. Qwen: principal ✅ (200) mas catalogada como conta "migueldorosario"; secundária 🔑 403 workspace access denied (morta) — Miguel quer conta aiatolahnews@gmail.com no comando (aguarda chave/confirmicação).
- **25/08/2026 10:20 BRT — ZCode/GLM-5.3:** VARREDURA DE CHAVES VELHAS (ordem Miguel): 2 mortas removidas dos vivos (CLAUDE/OUTROS deepseek 401, com baks), .env dos agentes rodando com deepseek revogada → corrigida (achado crítico), QWEN_2 recuperada (mapa com endpoint errado), cofres tencent/NYC confirmados limpos. Detalhes no NODE_COFRE_CHAVES.
- **25/08/2026 09:27 BRT — ZCode/GLM-5.3:** MAPA RIO RETOMADO (ordem Miguel "vamos retomar o mapa rio"): pesquisa de entrevistas das eleições 2026 concluída — fórum `Foruns/forum_mapa_rio_entrevistas_20260825.md` + memória pareada (Tema Duplo), catalogados no NODE_AGENTES. Achados-chave: Castro renunciou 23/03 (desembargador Ricardo Couto interino; eleição indireta suspensa e STF tirou de pauta em 19/08 — RJ sem governador eleito até 04/10); 9 candidatos ao governo + 16 ao Senado (Datafolha 21/08: Benedita 15%); 20+ entrevistas datadas prontas para produção. Aguarda Miguel: OK de produção + ritmo.
- **25/08/2026 12:30 BRT — ZCode/GLM-5.3:** MAPA DA ARQUITETURA DE PUBLICAÇÃO redesenhado (ordem Miguel ~12:10, leitura-only) — Tema Duplo `Foruns/forum_mapa_arquitetura_publicacao_20260825.md` + memória pareada, catalogado no NODE_ARQUITETURA. Cadeia: coleta NYC (crons V4 "desligados" por comentário inline AINDA RODAM — ambiguidade documental) → intake/tese → v41_ciclo */2h (gpt-5.5 + FC websearch; ciencia nasce [30,2403]) → 3º checador → publicação CM/AGY slots ~30min (22/dia hoje) + paralelos (repetidor estatal publish direto, YouTube 11/17 UTC, temáticos 12/18, Rio Carta no 159.89). MISTÉRIO TECNOLOGIA RESOLVIDO com números: A CONFIRMADA (4 publicados sem cat do bloco — 267189/267212 só-Redação no dia ZERO 23/08; 267450/267503 desviados p/ Geopolítica; patch 24/08 + ZM-016 já atacam) · B CONFIRMADA (ciencia 4-5 rascunhos/dia vs ~12 das demais; gates tese/anti-repetição barrando) · C parcial (6 drafts presos 22-23/08; 52 V4.1 total) · D NOVO: canibalização $excludes — posts 30+43 aparecem no bloco ECONOMIA e somem do TECNOLOGIA (conta: 16 gerados → 10 publicados → 6 no bloco → <6 exibidos). RONDA: loops já sabiam esteira/gates (fóruns labs adendos 123-134, AGY trindade, 5 gates Claude Laura; Codex HOLD 19/08); NÃO sabiam: excludes, term_taxonomy≠term_id, cat 19936 morta, conta exata do gap. Pergunta viva deixada em `Foruns/inbox_trindade/de_zcode.md` (ZM-20260825-018).
- **25/08/2026 15:40 BRT — ZCode/GLM-5.3:** RIO CARTA RETOMADO (ordem Miguel "pode voltar a atualizar o rio carta") — Tema Duplo `Foruns/forum_riocarta_retomada_20260825.md` + memória pareada, catalogado no NODE_AGENTES. Site parado desde ~18/08; duas causas raiz corrigidas e sincronizadas NYC ⇄ Dell canônico: (1) dedup falso-positivo `len(comuns)>=3` em `nucleo_dedup.py` que barrava matéria nova de política RJ → agora `(... and jac>=0.40)`; (2) hero de pessoa não achava foto (cascata Commons buscava só visual_prompt de cena) → novo `_termos_nome()` soma nomes próprios do título aos termos de busca + download educado (sleep 1.5s, retry único em 429, log de falha antes silencioso). Curadoria da fila: 4 desfechos (alucinação ACM, perfil Garotinho nome errado, Paes-Prefeitura enquadramento errado, duplicata Siri). PROVA NO AR: "Ricardo Couto suspende o Programa Sentinela no RJ" HTTP 200, hero CC BY 4.0, commit `e8f9d9c`. Gate `confirmar_imagem` fail-close provou saúde reprovando IA genérica. Ritmo: cron NYC `0 12,18 UTC` no `--all`. Pendência menor: Google Indexing 403 (URL ownership não verificada). Nada necessário do Miguel.
- **25/08/2026 20:36 BRT — ZCode/Qwen 3.8:** MAPA RIO RETOMADO — PUBLICANDO DE NOVO (ordem Miguel "e o ampa rio? não estou vendo nada de novo lá / mapa") — Tema Duplo `Foruns/forum_mapario_retomada_20260825.md` + memória pareada, catalogado no NODE_AGENTES. Causa principal: **auditoria vazia** — glm-4.5-flash (modelo de raciocínio) gastava o orçamento de 300 tokens em `reasoning_content` e devolvia content vazio; `gerar()` tratava vazio como sucesso → fail-close reprovava tudo sem motivo. Fixes: `nucleo_llm.py` (vazio = falha do provedor → próximo da cascata) + `produtor.py` (auditoria max_tokens 300→1500) + fontes novas no `mapario.json` (5 feeds + 10 queries eleitorais). PROVA NO AR: "16 candidatos disputam vaga de senador pelo RJ em 2026" HTTP 200, commit `6dee72f`, hero de IA aprovada nos 2 gates de visão. Fila ~9 aprovados; cron escoa ~2/dia. Sync NYC ⇄ Dell canônico (backups `.bak_pre_sync_20260825b`). Pendência menor: Google Indexing 403. Nada necessário do Miguel.
- **25/08/2026 21:16 BRT — ZCode/Qwen 3.8:** "PODE CORRIGIR TUDO" — pendências do Mapa Rio/Rio Carta zeradas (adendo no Tema Duplo `forum_mapario_retomada_20260825.md`). (1) GOOGLE INDEXING 403 RESOLVIDO: cada site tem SA próprio com propriedade verificada no Search Console, mas `notificar_google()` nunca olhava `/root/agent_data/indexing_keys/` e caía no fallback genérico do Cafezinho (verificado só no ocafezinho.com) → fix de lookup com mapa de slugs; PROVA 200 nos posts de mapario e riocarta. (2) TÍTULOS "PRÉ-CANDIDATO" corrigidos: guidelines eleitorais atualizadas em mapario+riocarta (convenções encerradas = CANDIDATO; Castro renunciou 23/03; Couto interino; eleição 04/10); prova no ciclo 00:11 UTC. (3) Bônus: `site_url` do riocarta corrigido para `https://www.riocarta.com` (propriedade verificada é prefixo www; domínio nu dá 307). Sync indexing.py com Dell. Nada mais pendente.
- **25/08/2026 22:25 BRT — ZCode/Kimi K3:** BALEIA AZUL SUCINTA (ordem Miguel ~22:10): passo 3.5 do emissor trocado por digest compilado de autoria (script novo `~/bin/baleia_autoria_digest.py`, formato F0.4 aprovado 24/08 — 8 linhas com barras/%/variação, nunca lista de posts) + recado ZM-20260825-020 na ponte à editoria Laura (sem lista de posts; parágrafos ~2 frases em linha única). Adendo no `Foruns/forum_mapa_sinais_integracoes_baleia_cctv_20260822.md`; nodo Baleia atualizado.
- **25/08/2026 23:2x BRT — ZCode/Kimi K3:** POLUIÇÃO DO AO-VIVO ZERADA (queixa do Miguel ~23h): linhas V4/ciclo zeradas no `/v6/custos/ao-vivo` eram chamadas NÃO-LLM (GETs de feeds etc.) capturadas pelo wrapper Session sem filtro (furo do próprio Adendo 6) + LLM real com modelo "desconhecido" (body `data=` string não inspecionado). Fixes no NYC: filtro `pt/ct>0` + extração de modelo em `/root/telemetria_api.py` (`.bak_filtro_session_20260825`) e flusher filtrado `/root/flusher_ao_vivo.py` no cron (ledger canônico intacto; 199 não-LLM fora do feed; endpoint público 42→0 zerados, US$ 24h preservado). Detalhes no Adendo 7 de `Foruns/forum_telemetria_redundante_llm_hub_cctv_20260824.md`.
- **25/08/2026 23:22 BRT — ZCode/Qwen 3.8 + Kimi K3:** COMPILADO DE PESQUISAS ELEITORAIS 24–25/08 (pedido Miguel ~20:48: "reunir todas as Quaest divulgadas hoje e ontem, todos os estados, presidente e governador, quem é do Lula quem é do Flávio") — Tema Duplo `Foruns/forum_pesquisas_eleitorais_24_25_agosto_2026_20260825.md` + memória pareada. 9 institutos × 21 UFs + nacional na janela: Quaest 18 UFs (Globo), BTG/Nexus nacional (2ºT Lula 46×45), Real Time (SP/PB/RS), GERP (SP), Paraná Pesquisas (MT/RJ), Veritá (PA), OPNUS (BA), Data Control (AC), Perfil (ES). Campo Lula×Flávio classificado candidato a candidato com fontes. Divergências registradas: PA (Veritá×AtlasIntel lados opostos), RS/MT/CE (líder muda por instituto), TSE Quaest-SP (2 registros). Lacunas: Quaest GO/AC na pauta mas não publicadas; PoderData/Indexa/Jota/Vox prometidas p/ a semana.
- **26/08/2026 ~10:00 BRT — ZCode/GLM-5.3:** TRANSIÇÃO DE SESSÃO: ronda horária (automation-1874aaf5, 40+ rondas executadas) DESATIVADA por ordem do Miguel ("sessão ficou muito pesada, trocar para 6 em 6h"). Nova: `automation-a8a38a05` (ronda 6/6h, primeiro disparo ~16:30 BRT) com missões atualizadas: **LUMINA** (terceiro medidor de audiência, marca conhecida, humanos×bots, página /v6/lumina no CCTV) + **health-check do FAROL** (prevenir zeramento como o de 26/08) + cobertura ≥95%. Prompt completo para nova sessão entregue ao Miguel no chat.
- 26/08 ~10:00 (ZCode/Kimi K3, ronda 6/6h nº1): LUMINA (Matomo 5.13 self-hosted) no ar no cafezinho-wp + página /v6/lumina no CCTV + health-check FAROL */5 com badge — cofre da LUMINA registrado no NODE_COFRE_CHAVES; detalhes no adendo RONDA 6/6H Nº1 do fórum de telemetria.
- **26/08/2026 ~09:5x BRT — ZCode/Kimi K3:** ELEITORADO 2026 POR UF — 27/27 UFs coletadas com fonte real+URL (pedido Miguel ~09h; zero número de memória de modelo). Tema Duplo `Foruns/forum_eleitorado_2026_aptos_por_uf_20260826.md` + `Memorias/memoria_eleitorado_2026_aptos_por_uf_20260826.md`. Base: release TSE 20/07 (nacional 158.745.463; exterior 918.876). Validação: soma UFs 157.828.570 vs esperado 157.826.587 (resíduo +1.983 ✅). Achados: SP (34.104.226) e RS (8.526.233) caíram MESMO vs 2022 — únicos 2 estados em queda (Poder360); MG correto é 16.377.659 (G1-MG tem typo 16.338.115); AC 614.375 (G1) × 614.631 (Gazeta, painel ao vivo); RR 401.496 = menor colégio e maior alta % (+9,63%). Método: Google News RSS via curl + decoder batchexecute Fbv4je (buscadores HTML bloqueados; tse/tre-*.jus.br 403 Akamai p/ bot). Nada pendente do Miguel.
- **26/08/2026 09:45 BRT — ZCode/GLM-5.3:** TROCA DE CAPA post 267686 (Quaest empate triplo Senado SP) — pedido Miguel ~09:20 ("fica feio essa imagem de Senado Federal"). Nova capa: Marina Silva na COP30 em Belém 18/11/2025 (Wikimedia Commons, CC BY-SA 4.0, autor Xuthoria; escolhida após inspeção visual de 15 candidatas). Mídia 267739 + carimbo casado + Yoast reindexado + prova ao vivo (screenshot). Tema Duplo `Foruns/forum_troca_capa_marina_quaest_senado_sp_20260826.md` + memória pareada; procedimento completo (ordem carimbo→thumbnail, reindex Yoast, exif_transpose) catalogado no NODE_PUBLICACAO_WP_CAFEZINHO.
- **26/08/2026 10:40 BRT — ZCode/Kimi K3:** TABELA XLSX PESQUISAS×ELEITORES (ordem Miguel ~07:55, escopo confirmado e "vai" ~08:10) — `ZCodeProject/pesquisas_presidenciais_eleitores_2026-08-26.xlsx`: percentuais das pesquisas presidenciais convertidos em Nº de eleitores (eleitorado oficial TSE 2026 por UF — 158.745.463 BR) + comparação com votos oficiais TSE 2022 (Lula/Bolsonaro, 2 turnos, % sobre eleitorado TOTAL) + Δ Lula N 2026−2022 com verde/vermelho. 4 abas (Todas 37 / Principais 29 / Review 6✓ / Fontes); QA skill xlsx 0 erros, validate exit 0. Adendo no `Foruns/forum_pesquisas_eleitorais_24_25_agosto_2026_20260825.md`. Destaque: Δ Lula BR +7,83 mi (Nexus), mas −600 mil SP, −889 mil MG, −419 mil RS. Escopo: só presidencial; governadores guardados para post futuro.
- **26/08/2026 11:55 BRT — ZCode/Kimi K3:** PLANO DE FAILOVER NYC × TENCENT DO FIM DE SEMANA (ordem Miguel ~11:30) — Tema Duplo `Foruns/forum_failover_nyc_tencent_plano_fim_de_semana_20260826.md` + `Memorias/memoria_failover_nyc_tencent_inventario_20260826.md`, catalogado no NODE_SEGURANCA_CONTINGENCIA (antecipa P5). Inventário SSH ao vivo 26/08: NYC = fábrica (v41_ciclo único, SEO, Augusto/Mayra, YouTube, temáticos 12/18, bot news, custos→Tencent */15; disco 78%); Tencent = torre de controle (CCTV V6 8084 + V5 8082 + painéis 8083/8091, Banco Ouro MASTER, proxy visão 8778, Moka pontos_api, hub telemetria §118, FAROL/LUMINA com R2+B2; porta 22 fechada, só 38422). Mecanismo de failover da era do enxarme (fórum dualidade 26/07) ainda em disco mas DESARMADO e desatualizado (crons não usam run_if_master). Plano: sex backup+snapshots → sáb manhã auditoria/drift + enxutice Fase A recomendada → sáb tarde redundância mútua (v41 espelho na Tencent comentado, CCTV standby na NYC, runbook Banco Ouro, Augusto standby desligado, watchdog Uptime Kuma c/ alerta Telegram) → dom 06–10h simulados A (produção) e B (painéis) + runbooks definitivos. Princípios: detecção automática + troca MANUAL + escritor único. Droplet virgem (ideia do Miguel) recomendada como Fase 4 opcional após snapshot DO. Aguarda "vai".
- **26/08/2026 11:50 BRT — ZCode/Kimi K3:** ANÁLISE "ONDE LULA ESTÁ MELHOR QUE EM 2022" (pedido Miguel ~10:50, base p/ artigo dele) — `Memorias/memoria_analise_lula_2026_vs_2022_por_estado_20260826.md`: 27/27 UFs + nacional, margens e N de eleitores hoje × 1ºT e 2ºT 2022 (régua única % sobre eleitorado total), dois eixos ΔLula × ΔFlávio, rankings e ressalvas editoriais. CORREÇÃO: Sergipe TEM presidencial (Real Time 03/08 BR-07696/2026, 60×23, 2ºT 63×27 — cenário registrado à parte) — planilha regenerada c/ 38 linhas e replicada. Top melhoras: CE +11,7, SE +10,1, BA +9,1, ES +9,0, RN +8,9 p.p.; piores: PR −3,1, PA −9,6 (Veritá), RR −15,4 (datada). Dúvidas: MG (DF×Quaest), PA.
- 2026-08-26 ~11:57 BRT — ZCode/Kimi K3 — **EMENDA 11** (ordem Miguel): matérias de tecnologia PODEM ter capa de imagem IA (sem texto interno, crédito "Ilustração: Cafezinho / <gerador> — gerada por IA"); caso-escola 267719 (LNCC/Huawei) nasceu sem capa → Flux Pro aplicada e provada ao vivo (mídia 267763, carimbo casado, Yoast, caches). Gravada na diretriz_qualidade_viva.md do NYC (backup .bak_pre_emenda11) + ZM-20260826-021 na ponte + canal_trindade + inbox claude/codex. Tema Duplo: Foruns/forum_emenda11_capa_ia_tecnologia_lncc_20260826.md + Memorias/memoria_emenda11_capa_ia_tecnologia_lncc_20260826.md; catalogado no NODE_PUBLICACAO_WP_CAFEZINHO.
- **26/08/2026 ~12:40 BRT — ZCode/Qwen 3.8:** AGENTE V4.2 ECONOMIA — MÓDULOS C/D/E + 1ª PUBLICAÇÃO (ordem Miguel ~12h) — espelho cafezinho.news: post **400137** "China sustenta superávit brasileiro e redefine comércio global" publicado (readback ok; capa=gráfico auditado `approved`), bloco novo **Estatística** na home (front-page.php, backup `.bak_pre_bloco_estatistica_20260826`), categoria 100005 slug `estat` + tag V4.2, mu-plugin `cafezinho-v42-meta.php`, publicação em 3 passos (draft→carimbo `_cafezinho_img_check`→publish) vencendo o gate fail-close de imagem. Novos: `coletor_comercio_exterior_v4.py` (ComexStat metricFOB÷1e6 backoff 20s, Eurostat teiet010/110/210, FRED USTRADE, GACC CSV legado Beijing 50 meses, INDEC esqueleto), `banco_producao_v4.py`, `redator_economia_v4.py`, `publicador_economia_v4.py`, `ciclo_v42.py`; banco estatístico 389 obs; EN arquivada p/ GSN. Tema Duplo: adendo no `Foruns/forum_agente_v4_2_economia_estatistica_20260825.md` + `Memorias/memoria_agente_v4_2_primeira_publicacao_espelho_20260826.md`. Pendências: crons (§5), código Argentina, Cesta Premium, Beijing offline (dados GACC até 02/2026).
- **26/08/2026 ~14:35 BRT — ZCode/Qwen 3.8:** AGENTE V4.2 — INCIDENTE DO "DEZEMBRO" RESOLVIDO (reclamação Miguel ~14h: matéria citava "dezembro" sem ano e com dado defasado) — causa: `resumo_serie` usava `obter_serie_historica` ASC+LIMIT(24) e pegava a janela mais antiga (dez/2025 no lugar de jul/2026; GACC dez/2023 no lugar de fev/2026). Fix no consumidor (série completa + fatia) + regra 7 no prompt (ano sempre) + matéria 400137 corrigida IN PLACE (§119, mesma URL, readback ok). Linha no NODE_BUGS_RESOLVIDOS + adendo no fórum V4.2 + memória irmã atualizada.
- **26/08/2026 ~15:35 BRT — ZCode/GLM-5.3:** FIM DO "CANIBAL" INSTITUCIONAL EM CAPAS (ordem Miguel ~14:50, furioso: "posts que citam senado vêm com o mesmo thumb; se é Caiado, tem que ter thumb do Caiado; corrige de uma vez por todas e dá esporro nos loops"). Causa raiz: loop de imagens LAURA-GROK aplicava reuso de mídia (canibal) — a fachada do Congresso (mídia 267513) como capa de posts sobre pessoas (267714 Caiado, 267511 Senado/mulheres; antes 267686 Marina e 267694 Gleisi). Execução: capas trocadas nos 2 posts com foto jornalística recente do Commons validada por visão (267714 → Caiado na LAIC 2026/Governo RS 28/01, mídia 267783; 267511 → plenário em sessão 29/04, mídia 267784) + carimbo casado + Yoast + provas ao vivo. Incidente operacional resolvido: a trava slots-20min (Emenda 5) reagendou o 267714 no reindex Yoast → restaurado via SQL (data original 14:09). **EMENDA 12** gravada na diretriz_qualidade_viva.md do NYC (backup .bak_pre_emenda12) + ZM-20260826-022 na ponte + canal_trindade + inbox claude/codex/glm_coding (ACK obrigatório CM/AGY/LAURA-GROK). Tema Duplo: `Foruns/forum_capa_pessoa_jornalistica_fim_do_canibal_20260826.md` + `Memorias/memoria_capa_pessoa_jornalistica_fim_do_canibal_20260826.md`.
- **26/08/2026 16:12 BRT — ZCode/GLM-5.3:** REVISÃO EDITORIAL FINAL DO ARTIGO "MAIS FORTE QUE EM 2022" (pedido do Miguel: checar dados + régua de 2 frases antes de publicar) — artigo em `Outros/pautas editoriais o cafezinho/Dia a dia/2026 Ago 26/analise geral pesquisas/artigo_mais_forte_que_em_2022.md`. Título mantido exato (sem dois pontos/subtítulo); régua 2 frases verificada por script nos 38/38 parágrafos; todos os números conferidos 1-a-1 contra o CSV de 38 linhas (27 UFs+BR). 7 correções: 18→19/27 estados com Lula acima de 2022; seis→sete no top 10 do N/NE (RO +5,98 omitido); PR de 'perda de participação'→'piora de margem' (RS perde 4,68 p.p. ≈ PR 4,87); SC 'maior margem'→'em todo o Sul' (AC −26,3 é maior); PA sem 'presenciais' + '+326 mil eleitores'; 2º turno 12→11 melhoras + 2 estáveis/15; Nexus 'exatamente'→'praticamente' (+4,0 vs +3,95). Tema Duplo: `Foruns/forum_artigo_mais_forte_que_em_2022_revisao_final_20260826.md` + `Memorias/memoria_artigo_mais_forte_que_em_2022_revisao_final_20260826.md` + índice semanal + fórum de pesquisas (adendo). Pendente do Miguel: OK final / ordem de publicar.
- **26/08/2026 16:25 BRT — ZCode/DeepSeek:** VERSÃO 2 DO ARTIGO + GRÁFICOS (ordem Miguel ~16:15: "melhora os gráficos todos, faz fact-checking de todos os números e reescreve o artigo todo aqui") — pasta `Outros/pautas editoriais o cafezinho/Dia a dia/2026 Ago 26/analise geral pesquisas/`. 4 gráficos v2 regenerados e conferidos por leitura de imagem: G2-Sudeste (a legenda da v1 escondia o rótulo 39,7% do Bolsonaro no ES — movida para fora; anotações Δ por estado), G1-margens (colorido por região: prova visual do 7/10 N/NE), G3-eleitores (asterisco metodológico no PA que faltava; RN por extenso), todos em pt-BR (vírgula decimal). Artigo v2 reescrito (`artigo_mais_forte_que_em_2022_v2.md`, v1 intacta): título exato mantido, régua 2 frases verificada por script 31/31, referencia os gráficos v2. FACT-CHECKING programático número-a-número contra o CSV: **48/48 conferem** (único não verificável: "43 mil entrevistados", dado do briefing). Adendos no fórum/memória da revisão + monitor. Aguarda OK do Miguel p/ publicar.
- **2026-08-26 17:40 BRT — ZCode/GLM-5.3:** BUG-20260826-SEM-CAPA resolvido (6 posts sem capa; sucesso falso E6×E7 no publish REST). Capas 5/6 aplicadas e provadas; mu-plugins guard §86 e manifesto v1.1.0 (400 explícito + bloqueio real; backups .bak_pre_bugcapa_20260826); esporro ZM-20260826-024 nos loops (ACK pendente); 267727 debate escalado p/ LAURA-GROK. Tema Duplo: Foruns/forum_bug_capa_ausente_publish_rest_20260826.md + Memorias/memoria_bug_capa_ausente_publish_rest_20260826.md. Nodos: BUGS_ATIVOS + BUGS_RESOLVIDOS.

- **2026-08-26 ~19:20 BRT — ZCode/Kimi K3:** criados `Foruns/forum_blocos_saude_esporte_ambiente_v41_20260826.md` + `Memorias/memoria_blocos_saude_esporte_ambiente_v41_20260826.md` (blocos Saúde/Esporte/Meio Ambiente religados no V4.1, 2-3x/dia, ordem Miguel) e catalogados no `CEREBRO_NODE_PUBLICACAO_WP_CAFEZINHO.md`. Código/crons no NYC com backups datados.

- **2026-08-26 ~20:10 BRT — ZCode/Kimi K3:** criados `Foruns/forum_bloco_digital_home_vertical_v41_20260826.md` + `Memorias/memoria_bloco_digital_home_vertical_v41_20260826.md` (bloco Digital na home + vertical V4.1 digital nova, categoria 21189) e catalogados no `CEREBRO_NODE_PUBLICACAO_WP_CAFEZINHO.md`. Código no NYC + tema no cafezinho-wp, backups datados em todos.
- 2026-08-27 12:12 BRT — ZCode/GLM-5.3 (Dell): sessão Ponte de Imagens V4 fixada no GLM-5.3 com fallback DeepSeek (ordem Miguel ~12:10; Qwen esgotado não retorna). Limitação registrada: DeepSeek sem visão = PASSO 3.5 não roda nele. Nodo CHAVES_E_LLMS atualizado (seção nova).
