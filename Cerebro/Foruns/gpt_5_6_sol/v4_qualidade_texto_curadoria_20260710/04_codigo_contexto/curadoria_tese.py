...
DEFAULT_CURADORIA_CONTRACT = "contratos/v4_curadoria_tese_v1.json"
...
class V4CuradoriaTese:
    ...
    def normalize_leitura_corrente(self, leitura: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "resumo_consenso": payload.get("resumo_consenso") or "",
    ...
    def _teses_candidatas(
        ...
        consequencia_text = str(consequencia).lower()
        has_external_pressure_anchor = (
            "tarifa" in consequencia_text
            or bool(re.search(r"\bustr\b", consequencia_text))
            or "washington" in consequencia_text
            or "trump" in consequencia_text
        )
        if has_external_pressure_anchor:
            tese_principal = f"Quando {entity} entra no centro da pressao externa, revela-se que {consequencia}."
        else:
            tese_principal = f"Quando os fatos auditados sobre {entity} saem do placar declaratorio, revela-se que {consequencia}."
        return [
            {
                "idx": 0,
                "tese": tese_principal,
    ...
    def _consequencia_material(source: dict[str, Any], primary_entity: str) -> dict[str, Any]:
        ...
        if "pix" in text:
            return {
                ...
                "quem_perde": f"{entity} e a direita alinhada a Trump",
        ...
        if "tarifa" in text:
            return {
                ...
                "quem_perde": f"{entity} e o campo politico associado a Trump",
        ...
        if "ibge" in text or "industria" in text or "indústria" in text or "setor produtivo" in text:
            return {
                "tipo": "economica",
                "quem_ganha": "leitor que acompanha a disputa sobre desenvolvimento produtivo",
                "quem_perde": "leitura que reduz industria a numero mensal sem estrutura produtiva",
                ...
                "evidencias_ref": ["fatos_travados:IBGE", "fatos_travados:setor_produtivo"],
                "nivel_concretude": 4,
            }
        if "jason miller" in text or "gettr" in text or "documentos judiciais" in text:
            return {
                "tipo": "institucional",
                "quem_ganha": "leitor que entende a rede de influencia politica em Washington",
                "quem_perde": f"{entity} e operadores que tratam pressao externa como militancia sem custo",
                "descricao": "os documentos judiciais deslocam a pauta de ataque retorico para responsabilidade institucional de atores ligados a Trump, Washington e eleicao brasileira",
                "evidencias_ref": ["fatos_travados:documentos_judiciais", "fatos_travados:Washington", "fatos_travados:Trump"],
                "nivel_concretude": 4,
            }
        return {
            "tipo": "institucional",
    ...
    def _collection_request(source: dict[str, Any], conteudo: str) -> dict[str, Any]:
        ...
        text = conteudo.lower()
        mentions_ustr = bool(re.search(r"\bustr\b", text)) or "escritorio do representante comercial" in text
