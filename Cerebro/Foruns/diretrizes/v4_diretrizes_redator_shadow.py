...
class V4ShadowRedator:
    ...
    def _text(curadoria: dict[str, Any]) -> str:
        ...
        paragraphs.append(
            f"Com esse deslocamento, {consequencia.get('quem_ganha')} ganha terreno político. "
            f"{consequencia.get('quem_perde')} fica exposto ao custo de uma pressão externa que já não cabe apenas na guerra de narrativas."
        )
