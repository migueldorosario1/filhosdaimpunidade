...
## Atualização após auditoria do Fable

Foi implementado o bloqueio de reabertura automática de decisões esgotadas. O agente agora mantém um contador append-only em `agent_data/v4/media/scout_run_budget.jsonl`, estaciona a pauta ao atingir três runs e exige `retry_reason` para uma retomada deliberada. O estado terminal é `autocura_esgotada`, sempre sem upload e sem autorização de publicação. A suíte de imagem e a compilação passaram após a alteração.
