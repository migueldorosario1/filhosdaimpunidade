...
def main() -> int:
    tests = [
        ...
        test_foruns_v4_nao_contem_fontes_vivas_e_pacote_auditoria_existe,
...
def test_foruns_v4_nao_contem_fontes_vivas_e_pacote_auditoria_existe() -> None:
    for forbidden in ["diretrizes", "v4_diretrizes", "v4_data"]:
        assert not Path("Cerebro/Foruns/v4", forbidden).exists(), f"fonte viva duplicada em Foruns/v4: {forbidden}"
    for required in ["diretrizes", "v4_diretrizes", "v4_data"]:
        assert Path(required).exists(), f"fonte viva ausente: {required}"
    package = Path("Cerebro/Foruns/v4/auditoria/pacote_auditoria_v4_fase2_standalone_20260709.tar.gz")
    if package.exists():
        with tarfile.open(package, "r:gz") as archive:
            names = set(archive.getnames())
        assert "pacote_auditoria_v4_fase2_standalone_20260709/diretrizes/v4_curadoria_tese_v1.json" in names
        assert "pacote_auditoria_v4_fase2_standalone_20260709/v4_diretrizes/test_contracts.py" in names
        assert "pacote_auditoria_v4_fase2_standalone_20260709/v4_data/curadoria/v4_real_001.curadoria.json" in names
    else:
        assert Path("AUDITOR_NOTE_FASE2_20260709.md").exists(), "pacote de auditoria ausente"
    flow = json.loads(Path("diretrizes/v4_fluxo_dry_run_v1.json").read_text(encoding="utf-8"))
    assert flow["steps"][0]["name"] == "curadoria_dry_run"
    assert flow["steps"][1]["from"] == "curadoria"
    agents = json.loads(Path("diretrizes/v4_agentes_tecnicos_v1.json").read_text(encoding="utf-8"))
