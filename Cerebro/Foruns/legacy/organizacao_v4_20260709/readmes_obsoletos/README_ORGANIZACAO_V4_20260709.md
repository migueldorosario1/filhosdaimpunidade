...
```text
Cerebro/Foruns/v4/ e apenas diretorio de discussao e documentos leves para colar em Fable/GPT 5.5/AGY.
Nao e fonte viva de execucao.
Nao deve conter copias descompactadas de `diretrizes/`, `v4_diretrizes/` ou `v4_data/`.
Nao deve conter pacote pesado se pudermos auditar diretamente o `v4_labs`.
...
Material de trabalho/auditoria tecnica atual:
...
Laboratorio operacional criado dentro do ecossistema Cafezinho:

```text
Projeto Cafezinho Agentes/root/v4_labs/
```

Destino final reservado para arquivos auditados:

```text
Projeto Cafezinho Agentes/root/v4/
```

Qualquer mudanca de codigo, contrato ou dado de trabalho deve ser feita nas fontes vivas e sincronizada com `v4_labs` enquanto estivermos em fase de laboratorio. Para auditoria externa, preferir apontar Fable/GPT 5.5 diretamente para `Projeto Cafezinho Agentes/root/v4_labs/`.
