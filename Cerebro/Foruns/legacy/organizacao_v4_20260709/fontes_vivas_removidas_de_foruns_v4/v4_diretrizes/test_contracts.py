...
def test_espelho_diretrizes_reconciliado_com_fonte_viva() -> None:
    pairs = [
        ...
        ("diretrizes/v4_redator_shadow_v1.json", "Cerebro/Foruns/v4/diretrizes/v4_redator_shadow_v1.json"),
    ]
    ...
    flow = json.loads(Path("Cerebro/Foruns/v4/diretrizes/v4_fluxo_dry_run_v1.json").read_text(encoding="utf-8"))
    assert flow["steps"][0]["name"] == "curadoria_dry_run"
    assert flow["steps"][1]["from"] == "curadoria"
    agents = json.loads(Path("Cerebro/Foruns/v4/diretrizes/v4_agentes_tecnicos_v1.json").read_text(encoding="utf-8"))
