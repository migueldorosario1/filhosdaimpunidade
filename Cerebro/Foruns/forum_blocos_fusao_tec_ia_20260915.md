...
## ➕ Adendo — PROPOSTA DO MIGUEL: «vertical preenchedor de lacunas» (15/09 ~23:2x)

Ideia dele: além do Telegram, o alerta de bloco parado DEFLAGRA um agente autônomo que (1) diagnostica o problema, (2) busca nos bancos de notícias, (3) se banco seco faz coleta, (4) produz e publica material para o bloco lacunoso. Parecer ZM: FAVORÁVEL — a matéria-prima EXISTE parada (bancos v4_verticals quentes: cultura 22MB c/ bloco a 150h; caso de ontem: pauta OpenAI/Anthropic reprovada com nota 6,29 alta); o gargalo é o juiz/ciclo, não a coleta. Desenho proposto (nome jornalístico: TAMPÃO — matéria que fecha buraco; script vertical_tampao_v1.py): gatilho = vigia existente; fase-diagnóstico (categoria errada? esteira presa? nada aprovado?) → fase-bancos (melhores não-publicados da vertical pro ciclo V4.1) → fase-coleta (coletor sob demanda) → publicação pela esteira V4.1 COM os gates da casa (nunca pular juiz — anti-lixo); guardas: máx 2/bloco/dia, custo ~centavos (DeepSeek), log + Telegram de resultado. AGUARDA «vai» do Miguel + decisão: publica direto (recomendado — rascunho não enche bloco) × entrega rascunho.

— ZCode/GLM-5.3 · 15/09/2026 23:2x BRT · adendo proposta Tampão
