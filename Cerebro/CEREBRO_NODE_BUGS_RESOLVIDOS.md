...
## BUG-20260915-YM-FAILOPEN-TRANSCRICAO — ✅ RESOLVIDO no plantão (15/09/2026 ~19:2x BRT)

**Sintoma:** 1º pedido real pelo painel /v6/youtube (Poll FLIP/Breaking Points): tabela mostrava ✅✅ «concluido», mas a transcrição tinha FALHADO (0 chars — Transkriptor status=Failed em 60s; fallback yt-dlp bloqueado por bot-check do YouTube no IP NYC) e o executor redigiu mesmo assim — a LLM **alucinou aspas** de Krystal Ball e Saagar Enjeti (draft 271151 PT + post EN do GSN). Agravante: push do GSN rejeitado (remote à frente) era logado como «publicado».

**Causa raiz:** dois fail-opens no youtube_manual.py: (1) transcrição vazia seguia o fluxo sem base factual (bloco de transcrição simplesmente ausente do prompt); auditoria da casa (título/idioma/tamanho/entrevistado citado) não detecta aspas alucinadas; (2) `git push` com `check=False` sem inspecionar retorno.

**Cura (no ar):** patch fail-closed — transcrição <500 chars ⇒ aborta o pedido inteiro com 🔴 por destino («transcrição indisponível — nada produzido»); push verificado ⇒ RuntimeError com stderr. Limpeza: matéria fabricada descartada do GSN (fetch+reset p/ origin/main, backup em agent_data/youtube_manual/descartados/) e do WP (271151 → lixeira, backup /root/descartados_yt_20260915/). Reexecução do pedido: transcrição real 21.551 chars (US$ 3), draft 271200 + post GSN no ar 200.

**Lições:** (1) Transkriptor é intermitente no MESMO vídeo (Failed→Completed com 4h de diferença) — retry é primeira resposta; (2) veredicto ✅ de máquina sem prova de insumo (transcrição) não vale nada; (3) 🔴 residual: gate automático de aspas (validar citações contra a transcrição) proposto no fórum v1.2, aguarda «vai». Fórum: Foruns/forum_painel_comando_youtube_manual_20260915.md adendo v1.2.
...
### ✅ RESOLVIDO 07/09 16:09 BRT · BUG-20260907-DATA-DIA — «neste domingo (7)» publicado numa segunda-feira (post 269341)
- **Causa (4 elos):** texto humano errou o dia da semana na confecção; R1 não cobre o autor 5780 (filtro de automáticos) e ainda teve 2 ciclos mortos na janela; R2 não tinha noção de calendário (prompt sem dia da semana); publicação humana sem bloqueio e sem varredura pós-publicação.
- **Cura pontual:** «neste domingo (7)» → «nesta segunda-feira (7)» (editorial Estadão «Cronicamente mendaz» saiu segunda 07/09 03:02 BRT — Google News RSS do publisher + Brasil 247 06:01). Prova pública 5× novo / 0× velho.
- **Cura estrutural (ordem Miguel "apertando juízes, revisores, publicador externo"):** gate mecânico `data_semana_gate.py` (22/22 testes) plugado em R1 (todos os autores), R2 (pendência DATA×DIA força ok=false), publicador DS-N (bloqueante), juiz 2 V4.1 (reprovação de forma c/ salva-drafts) + sweeper pós-publicação */15 no cafezinho-wp (rede final p/ humano; ronda Vigia :12 consome flags → Telegram). Bônus na mesma janela: revisores varrem draft+future (achado A), anti-loop por sha1 (B), gpt-5 budget 4000 (D), try/except gravar (E), NameError tag (F).
- **Prova real:** o gate flaga o texto original do 269341 («dia 7 mais próximo de 07/09/2026 é 07/09/2026, uma segunda-feira») e não flaga o corrigido.
- **Backups/rollback:** `.bak_pre_datasemana_20260907` (tencent×3, NYC) + linha cron DATASEMANA_SWEEPER_20260907 (cafezinho-wp). Detalhes: `Foruns/forum_gate_data_dia_semana_269341_20260907.md` + `Memorias/memoria_gate_data_dia_semana_269341_20260907.md`.
- **Lições:** (1) filtro de autor nunca pode excluir o GATE MECÂNICO gratuito (ele não custa nada e pega o que o LLM não vê); (2) prompt de revisor LLM precisa dizer o DIA DA SEMANA de hoje, não só a data; (3) publicação humana precisa de rede pós-publicação (sweeper), não só de portões pré; (4) §112: reler o monitor IMEDIATAMENTE antes de escrever em produção em sessão longa (colisão v41_ciclo.py 16:24×16:29, curada por merge).

- **07/09/2026 — "controle" vazando no HTML público (ordem Miguel: "não pode ter link controle na frente")** — causa raiz: filtro `admin_url → new_admin_url` do plugin serverdoin-cdn registrado INCONDICIONAL (reescrevia www→controle até no front anônimo; ex.: ajaxUrl do WP Statistics). Cura: guard contextual (admin/preview/host-controle/logado) + siteurl/home do DB → www + purga de caches. Provas: home 0 ocorrências, ajaxUrl público, HTTP 200. Ver [fórum](./Foruns/forum_controle_admin_url_frontend_estrutural_20260907.md).

- **08/09/2026 — Vazamento de scaffolding LLM em post publicado (caso 269519 + 267466; ordem Miguel "corrigir urgente + investigação profunda + cura estrutural")** — Gabriel (user 5735) cola a resposta BRUTA do LLM pessoal dele no wp-admin (o prompt dele pede «Bloco "Leia Mais"» + 5 «Opções alternativas de título») e publica em ~4 min do celular/PC; template inexistente no ecossistema (grep 0 hits). Nenhum gate cobria: publicação humana direta (§130), R1/R2 crons :05/:20 vs janela de 4 min. Provas: `_cafezinho_origem` (via=admin, UA Android 269519 / Windows 267466) + `_edit_last=5735`. Fix: override CAFEZINHO_EDITORIAL_HUMAN_OVERRIDE (ordem explícita) + update POSICIONAL (`wp post update <id> <arquivo>` — 🔴 `--post_content=@` grava string literal) + purge completo; URLs canônicas 200 limpas (0 travessão/0 bloco), travessões do 269519 removidos a pedido do Miguel. Draft 268459 (mesmo padrão, metatexto "Segue a matéria") não tocado — filtro limpa no próximo save. Cura em 3 camadas: (1) mu-plugin `cafezinho-filtro-scaffolding.php` no save (7/7 testes PASS; corta blocos de scaffolding + metatexto de abertura, log em option, notice amarelo; nunca toca status/date/autor) · (2) sweeper `/root/verificador_vazamento.py` cron */15 no cafezinho-wp (marker VAZAMENTO_SWEEPER_20260908; flags bloco_interno/metatexto/markdown_cru/travessoes; NUNCA edita post) · (3) Ronda ZM Vigia passo P2.6 consome flags → Telegram (🔴 bloco/metatexto; 🟡 travessões). Fact-check: conteúdo REAL (Quaest/SP 08/09 — Bing News RSS bateu 1:1); "estranho" era forma, não fato. Pendentes aguardam "vai" do Miguel (retroativo travessões ~58 posts, 269523, manual, recado ao Gabriel, fact-check humano, travessão em R2/juiz2). Ver [fórum](./Foruns/forum_vazamento_scaffolding_269519_20260908.md) + [memória](./Memorias/memoria_vazamento_scaffolding_269519_20260908.md).

- 08/09/2026 — Rio Carta: hero de pessoa de OUTRO estado (Aécio Neves/MG em matéria do Senado RJ) + título title-case americano — RESOLVIDO: tribunal visual (gate_credito_nomes custo zero + 2 juízes de visão c/ regra geo/pessoa + desembargador fail-close, opt-in `tribunal_visual`) + `_sentence_case_ptbr` no produtor; fix ao vivo commit 180320f. Fórum: Foruns/forum_tribunal_visual_riocarta_20260908.md

- 09/09/2026 — GSN: player de YouTube DUPLICADO em 21 briefs (consumidor gsn_fila prefixava embed_youtube sobre conteudo_html que já vinha com embed do auditor) — RESOLVIDO: guarda no consumidor + dedup do acervo commit c7e3195; provas ao vivo 1 iframe/post. Fórum: Foruns/forum_gsn_embed_duplicado_20260909.md

- 09/09/2026 — AIATOLAH BILÍNGUE: página /en/ com título inglês × corpo 100% português (caso 20260825 auto-mode Claude Code; ordem Miguel "solução estrutural") — RESOLVIDO: causa-raiz TRIPLA no `publicador._traduzir()` do V4 NYC (fail-open por campo com max_tokens 4096 comido por reasoning/família BUG-DS-102 + atalho `lingua_alvo=='pt'→cópia cega` que transformava artigo nascido EN em versão "PT" inglesa + zero revalidação pós-tradução; bônus: hero_legenda nunca traduzida → ~38 legendas PT visíveis em página EN). Extensão: 11/403 posts (9 EN c/ corpo PT + 2 PT c/ conteúdo EN). Cura: V4_PATCH_BILINGUE_IDIOMA_20260909 (detectores `_parece_ingles`/`_parece_portugues_geral` no produtor — "as" FORA da lista EN; gate `_versao_no_idioma` pré-write em TODAS as versões; `_traduzir` fail-closed retry×2 + max_tokens 8000 + validação de campos/comprimento ≥40%/cercas de código/idioma; publicação ADIADA quando nenhuma versão sobrevive; alertas Telegram throttleados) + retrocorreção `retro_idioma_20260909.py` (11 corpos deepseek + 38 legendas determinísticas + 2 títulos/excerpts PT; commit 5f0d5e3 push→Vercel; slugs intactos SEO; backups 48 arquivos em /root/tematicos/agent_data/backup_idioma_20260909). Provas: teste 403 posts local+remoto 11/11 flagrados 0 falso-positivo; pós-retrofit 0 flags; AO VIVO verificado (auto-mode frase EN exata no ar, wasmtime HTTP 200 c/ 3 blocos de código, sam-altman PT, muse-glimmer legenda EN). Rollback: .bak_pre_bilingue_20260909 ×2 (md5 na memória) + git revert 5f0d5e3. Prova do patch em corrida real = cron 12:00 UTC (09:00 BRT). Lições: (1) update por campo com `if json.get(k)` é fail-open — campo obrigatório ausente tem que REJEITAR a resposta inteira; (2) atalho de "tradução" nunca pode presumir idioma do original — DETECTAR; (3) gate de idioma tem que rodar DEPOIS da tradução, pré-write, em TODAS as versões; (4) título não é autoridade de idioma (loanwords legítimos) — o CORPO é; (5) "as" é o único overlap function-word PT×EN. Fórum: Foruns/forum_aiatolah_bilingue_corpo_idioma_errado_20260909.md + Memorias/memoria_aiatolah_bilingue_corpo_idioma_errado_20260909.md

## [2026-09-14] BUG-20260914-ZM-004 — «Robinson Farinazzo» no lugar de «os Estados Unidos» nos rascunhos de Vídeos (decupagem YouTube)
- **Sintoma:** 6 rascunhos cat 28 (270857/270616/270613/270612/270520/270519) + 1 na tabela produzidos com nome de pessoa substituindo sujeito institucional (flagrado pela CL no CL-20260914-010).
- **Causa raiz:** lista GLOBAL de personagens canônicos (`personagens_youtube.json`, inclui «Robinson Farinazzo»/TV 247) injetada no prompt de TODO vídeo + regra que OBRIGAVA a nomear apresentador → LLM puxa nome da lista quando o vídeo não tem apresentador nomeado.
- **Fix:** escopo do bloco GRAFIA CANONICA restrito a grafia de nomes PRESENTES no vídeo + regra 10 condicionada (sem nome no vídeo → descrever papel, nunca usar a lista). Backup `.bak_pre_farinazzo_20260914`.
- **Lição:** lista de grafia canônica com autoridade de «casa» + regra imperativa de nomear pessoas = alucinação dirigida; listas de referência no prompt devem ter escopo EXPLÍCITO de uso.
- Fórum: `Foruns/forum_bug_farinazzo_decupagem_20260914.md` — ZM · ZCode/GLM-5.3

## 15/09/2026 — Top 10 Tendências: velocidade calculada sobre tabela GA4 truncada (resolvido)

`top_tendencias_push.py` (NYC, cron horário) consultava pagePath×dateHour com `limit=1000` sem filtro; a tabela real tem 6.520 linhas → GA4 cortava silenciosamente e a "velocidade 6h" era lixo estatístico há semanas (v/h inventados). Junto: NameError 'fonte' congelava a página /top10 desde a v3 (24/08) e idade dos posts inflada +3h (date local BRT tratada como UTC). Cura v4: filtro BEGINS_WITH "/20" (posts) + limit 10000 + janela real 48h por hora + date_gmt + fonte por parâmetro. Provas e detalhes: Foruns/forum_auditoria_top10_velocidade_v4_20260915.md. — ZM/GLM-5.3 15/09/2026
