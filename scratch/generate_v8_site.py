...
Nota conjunta conclamando a liderança conservadora americana a condicionar qualquer diálogo comercial à anistia ampla dos réus de Brasília.`
    ...
    function makeLastRevisionCanonical(sourceBtn) {
      ...
      const ch = dataset[currentChapterKey];
      if (ch) {
        // QA-FIX NOME CANÔNICA (Kimi 3, 2026-08-05): a canônica vira PONTEIRO para
        // a versão de origem — o NOME original nunca é apagado (pedido do Miguel:
        // "depois que torna canônica mantém o nome, não tira o nome").
        ...
        const canonWriteRes = safeLocalSet('miguel_book_persistent_content_' + currentVolume + '_' + currentChapterKey, finalTxt);
        const canonKeyRes = safeLocalSet(storageKeyVol('miguel_book_canonical', currentChapterKey), originKey);
        saveCustomChapters();
        saveChapterPersistentState(currentChapterKey, finalTxt, originLabel);
        renderSingleView();
        ...
        if (canonWriteRes.ok && canonKeyRes.ok) {
          ...
          alert(`👑 O Capítulo "${ch.title}" agora tem como versão CANÔNICA:\n\n"${originLabel}"\n\nO nome original da versão foi preservado no menu de versões.`);
        } else {
