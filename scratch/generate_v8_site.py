...
<body class="min-h-screen flex flex-col antialiased bg-[#f6f4ee] text-slate-900 selection:bg-amber-300 selection:text-slate-900">
  ...
  <script>
    ...
    async function callRealLlmApi(engineKey, instruction, originalText, consultMemory) {
      const keys = {
        gemini: localStorage.getItem('miguel_key_gemini') || '',
        gpt56: localStorage.getItem('miguel_key_openai') || '',
        opus5: localStorage.getItem('miguel_key_anthropic') || '',
        deepseek: localStorage.getItem('miguel_key_deepseek') || 'sk-9335ff9479ea4c2eba10bd0c498dde04',
        kimi35: localStorage.getItem('miguel_key_kimi') || 'sk-e2j7OPypKuX0GfOzr0Atm4zjjJZ3aUIQeaQ892Dm88iSqfaR',
        glm52: localStorage.getItem('miguel_key_glm') || ''
      };
...
2. Preserve a integridade factual dos eventos descritos.`;
      ...
      try {
        if (engineKey === 'glm52' && keys.glm52 && !keys.glm52.includes('default')) {
        ...
        } else if (engineKey === 'gemini' && keys.gemini && !keys.gemini.startsWith('sk-3b473538')) {
