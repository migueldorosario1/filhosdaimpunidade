...
<body class="min-h-screen flex flex-col antialiased bg-[#f6f4ee] text-slate-900 selection:bg-amber-300 selection:text-slate-900">
  ...
  <script>
    ...
    function analyzeInstructionAndGenerateObjectiveReport(instruction, summaryRule, engineName) {
      ...
      return `
        <div class="space-y-3 font-sans text-sm text-slate-900">
          <div class="flex items-center justify-between text-slate-900 font-bold text-sm border-b border-amber-300 pb-2">
            <span class="flex items-center gap-2">
              <i data-lucide="check-circle-2" class="w-4 h-4 text-emerald-700"></i>
              <span class="text-slate-900 font-extrabold">Relatório objetivo de alteração (${engineName}):</span>
            </span>
          </div>
          <div class="bg-amber-50/80 p-4 rounded-2xl border border-amber-300 space-y-2.5 text-sm text-slate-900 leading-relaxed shadow-sm">
            <div><strong class="text-purple-950 font-extrabold">1. Entendimento do pedido:</strong> "${instruction}"</div>
            <div><strong class="text-emerald-950 font-extrabold">2. Ação objetiva executada:</strong> ${actionDetail}</div>
            <div><strong class="text-amber-950 font-extrabold">3. Diretriz de estilo registrada:</strong> <em>"${summaryRule}"</em></div>
          </div>
          <div class="pt-1">
            <button onclick="scrollToAndHighlightRewrittenText()" class="w-full py-2.5 px-4 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-extrabold uppercase tracking-wider flex items-center justify-center gap-2 shadow transition cursor-pointer">
...
2. Preserve a integridade factual dos eventos descritos.`;
      ...
      try {
        ...
        console.warn("Real API call failed or missing API key, falling back to local surgical engine:", err);
      }
      return null;
    }
    ...
    async function runDeepSeekV4Instruction() {
      ...
      // Attempt real API call to selected LLM provider (GLM 5.2, DeepSeek, Kimi, Gemini, OpenAI, Claude)
      let rewrittenText = await callRealLlmApi(currentSelectedEngine, instruction, origContent, consultMemory);
      ...
      if (badgeElem) {
        badgeElem.innerHTML = `<i data-lucide="check-circle-2" class="w-4 h-4 text-emerald-700"></i> <span>Resultado da revisão (${currentEngineName}):</span>`;
      }
