...
<body class="min-h-screen flex flex-col antialiased bg-[#f6f4ee] text-slate-900 selection:bg-amber-300 selection:text-slate-900">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Render static modals content
      document.getElementById('banco-links-content').innerHTML = marked.parse(bancoLinksMarkdown);
    ...
    function renderMetrics() {
      ...
      if (currentChapterKey === 'full_book') {
        ...
        document.getElementById('metric-author').textContent = fullData.author;
        document.getElementById('metric-version-tag').textContent = fullData.versionTag;
        document.getElementById('metric-words').textContent = fullData.totalWords.toLocaleString('pt-BR');
        document.getElementById('metric-sentences').textContent = `${fullData.totalChapters} Caps`;
        document.getElementById('metric-badge').textContent = `Compilado Canônico (${fullData.totalChapters} Capítulos)`;
        return;
      }
      const activeData = getActiveVersionData();
      const metrics = calculateMetrics(activeData.content);
      
      document.getElementById('metric-author').textContent = activeData.author || 'Kimi (ZCode) & Miguel';
      document.getElementById('metric-version-tag').textContent = activeData.versionTag || dataset[currentChapterKey].versionTag;
      document.getElementById('metric-words').textContent = metrics.words.toLocaleString('pt-BR');
      document.getElementById('metric-sentences').textContent = metrics.sentences.toLocaleString('pt-BR');
      document.getElementById('metric-badge').textContent = activeData.badge || 'Oficial Protegida 🔒';
    }
